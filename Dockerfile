...
FROM ubuntu:22.04 as builder_base_idais
...
FROM builder_base_idais as python_libs_idais
WORKDIR /app
...
COPY .git ./.git
COPY govapp ./govapp
RUN cd /app/govapp/frontend/idais; npm install
RUN cd /app/govapp/frontend/idais; npm run build
...
CMD ["/startup.sh"]
LABEL org.opencontainers.image.source="https://github.com/dbca-wa/idais"
